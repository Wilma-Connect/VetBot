...
<body class="bg-[#faf3e8] text-white min-h-screen items-center justify-center p-4">
    ...
    <div class="flex items-center justify-center min-h-screen">
        ...
        <form action="{{ route('vetbotconseil.start') }}" method="POST" class="bg-white text-black w-full max-w-md p-6 rounded-lg shadow space-y-4">
            @csrf
            <div class="logo flex justify-center">
                <img src="assets/vetbot-logo.png" alt="VetBot Logo" class="w-16 h-auto">
                <!-- <h1>VetBot</h1> -->
            </div>
            <h1 class="text-xl font-bold text-center">Conseils</h1>
    ...
    <div class="header-nav">
      <div class="flex items-center gap-3">
        <select id="pageSelect" class="page-select">
          ...
          <option value="{{ route('conseil') }}" >Conseils</option>
  ...
  <div class="p-4 border-t border-[#181818] fixed bottom-0 left-0 right-0">
    ...
    <form action="{{ route('vetbotconseil.send', $conversation) }}" method="POST" enctype="multipart/form-data" id="chatForm" class="flex flex-col gap-2 p-2 bg-green-800/30 rounded-md">
        @csrf
        ...
        <textarea id="chatInput" name="content" type="text" class="p-2 flex-1 bg-white rounded text-gray-700 text-sm resize-none" placeholder="De quels conseils avez-vous besoin sur l'élevage ?" rows="5" ></textarea>
